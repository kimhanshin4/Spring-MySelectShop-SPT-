...
@WebMvcTest(
    controllers = {UserController.class, ProductController.class}, //테스트 할 Controller 지정
    excludeFilters = {
        @ComponentScan.Filter( //제외시킬 Filter 지정
            type = FilterType.ASSIGNABLE_TYPE,
            classes = WebSecurityConfig.class //왜 제외를 하지?
        )
    }
)
class UserProductMvcTest {
    ...
    public void setup() {
        mvc = MockMvcBuilders.webAppContextSetup(context)
            .apply(springSecurity(new MockSpringSecurityFilter())) // 생성한 가짜 필터=>가자Spring필터
            .build();
    }
    ...
    void test3() throws Exception {
        ...
        mvc.perform(post("/api/products")
                .content(postInfo) //String Type
                .contentType(MediaType.APPLICATION_JSON) //String타입의 JSON
                .accept(MediaType.APPLICATION_JSON) //받을 때도 JSON
                .principal(mockPrincipal) //가짜 객체 회원
            )
            .andExpect(status().isOk()) //기대응답코드
            .andDo(print()); //
    }
}